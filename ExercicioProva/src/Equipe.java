...
public class Equipe {
    ...
    private boolean trocarPneu(int tipoDePneu){
        if(tipoDePneu>=1 && tipoDePneu <=5 ){
            ...
            System.out.println("Seu pneu agora eh do tipo" + carro.getTipoDePneu());
            return true;
        }
        else{
            return false;
    ...
    public void adicionarPiloto(Piloto piloto){
        if(pilotoList.size() < 4){
            ...
            System.out.println(piloto.getNome() + " adicionado a equipe");
            
        }
        else{
            System.out.println("Limite maximo de pilotos atingido");
    ...
    public boolean retirarPiloto(int pilotoId){
        if(pilotoId > 0 && pilotoId <= pilotoList.size()){
           pilotoList.remove(pilotoId -1);
            System.out.println("Piloto foi removido");
           return  true;
        }
        else{
